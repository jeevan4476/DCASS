...
def _is_path_like(value: str) -> bool:
    ...
    return (
        ...
        or "\\" in value
        or value.endswith(
            (".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp", ".mp3", ".wav", ".flac")
        )
    )
...
class MediaItem:
    ...
    _file_path: Optional[str] = field(default=None, repr=False, compare=False)
    _file_path_resolved: bool = field(default=False, repr=False, compare=False)

    @property
    def file_path(self) -> Optional[str]:
        """Resolve exact absolute local file path on disk (cached after first call)."""
        if self._file_path_resolved:
            return self._file_path
        self._file_path = self._resolve_file_path()
        self._file_path_resolved = True
        return self._file_path
    ...
    def _resolve_file_path(self) -> Optional[str]:
        ...
        if self.modality == "image":
            ...
            raw_path = (
                self.metadata.get("file_path")
                or self.metadata.get("path")
                or self.metadata.get("text_path")
            )
            if raw_path:
                p = Path(raw_path)
                if p.is_absolute() and p.exists():
...
class UnifiedSemanticIndex:
    ...
    def __init__(
        ...
        # id -> (modality, local_index) lookup built at load time
        self._id_lookup: dict[str, tuple[Modality, int]] = {}
    ...
    def load(self, modalities: list[Modality] = None) -> dict[str, bool]:
        ...
        for modality in modalities:
            ...
            try:
                ...
                # Build id lookup for O(1) get_by_id
                for i, meta in enumerate(self.metadata[modality]):
                    media_id = meta.get("id")
                    if media_id is not None:
                        self._id_lookup[media_id] = (modality, i)
    ...
    def get_by_id(self, item_id: str) -> Optional[MediaItem]:
        ...
        """
        hit = self._id_lookup.get(item_id)
        if hit is None:
            return None
        modality, idx = hit
        meta_list = self.metadata.get(modality, [])
