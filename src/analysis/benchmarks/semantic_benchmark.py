...
class SemanticBenchmark:
    ...
    def __init__(
        ...
        if dataset_path is None:
            # Must match GET /api/benchmark/latest, which reads
            # storage/data/benchmarks/results/.
            dataset_path = (
    ...
    def metrics(self) -> CombinedMetrics:
        ...
        if self._metrics is None:
            ...
            self._metrics = CombinedMetrics(
                device=self.device, bertscore_model=self.bertscore_model
            )
    ...
    def run_single(
        self, message: str, category: str, diversity_mode: DiversityMode
    ) -> SampleResult:
        """
        ...
        return SampleResult(
            ...
            bertscore_precision=bert_result.details.get("precision", 0.0)
            if bert_result
            else 0.0,
            bertscore_recall=bert_result.details.get("recall", 0.0)
            if bert_result
            else 0.0,
    ...
    def run(
        ...
        if categories:
            all_messages = [
                (cat, msg) for cat, msg in all_messages if cat in categories
            ]
        ...
        for mode_idx, mode in enumerate(modes):
            ...
            for i, (category, message) in enumerate(all_messages):
                if verbose:
                    ...
                    print(
                        f'  [{progress:5.1f}%] {category}: "{message[:40]}{"..." if len(message) > 40 else ""}"'
                    )
                ...
                try:
                    ...
                    if verbose:
                        print(
                            f"          CLIP: {result.clip_similarity:.3f} | BERT: {result.bertscore:.3f}"
                        )
    ...
    def _compute_statistics(
        self, samples: list[SampleResult], modes: list[str], total_time: float
    ) -> BenchmarkResults:
    ...
    def save_results(self, results: BenchmarkResults, path: Path = None) -> Path:
        ...
        Args:
            ...
            path: Output path. Defaults to storage/data/benchmarks/results/
            (the directory GET /api/benchmark/latest reads).
...
def run_benchmark(
    modes: list[DiversityMode] = None, verbose: bool = True, save: bool = True
) -> BenchmarkResults:
    """
