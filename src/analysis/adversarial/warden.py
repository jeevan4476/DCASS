...
class DeepPacketInspectionWarden(nn.Module):
    ...
    def extract_statistical_features(self, delays: torch.Tensor) -> torch.Tensor:
        ...
        if delays.size(1) > 1:
            ...
            cov = ((delays_t - mean_t) * (delays_t1 - mean_t1)).mean(dim=1, keepdim=True)
    ...
    def forward(
        ...
        Args:
            ...
            channel_ids: Channel indices [0, num_channels-1], shape (batch_size, sequence_length)
        ...
        stat_features = self.extract_statistical_features(delays)  # (batch_size, feature_dim)
        ...
        # Embed channels
        channel_embed = self.channel_embedding(channel_ids)  # (batch_size, seq_len, 32)

        # Combine all embeddings
        combined = torch.cat(
            [delay_embed, channel_embed, stat_features_expanded],
            dim=2
        )  # (batch_size, seq_len, 64+32+feature_dim)

        # Project to hidden dimension
        sequence_features = self.input_projection(combined)  # (batch_size, seq_len, hidden_dim)

        # Temporal modeling with BiLSTM
        lstm_out, _ = self.bilstm(sequence_features)  # (batch_size, seq_len, hidden_dim)

        # Global pattern recognition with Transformer
        transformer_out = self.transformer(lstm_out)  # (batch_size, seq_len, hidden_dim)

        # Per-timestamp anomaly scores
        anomaly_scores = self.anomaly_head(transformer_out).squeeze(-1)  # (batch_size, seq_len)
...
def compute_warden_loss(
    ...
    real_loss = nn.functional.binary_cross_entropy(
        real_verdict.bot_probability,
        real_target
    )

    fake_loss = nn.functional.binary_cross_entropy(
        fake_verdict.bot_probability,
        fake_target
    )
...
if __name__ == "__main__":
    ...
    print(f"✓ Suspicious real samples: {real_verdict.is_suspicious().sum().item()}/{batch_size}")
    print(f"✓ Suspicious bot samples: {bot_verdict.is_suspicious().sum().item()}/{batch_size}")
